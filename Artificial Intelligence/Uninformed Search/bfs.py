...
def bfs(state, i, j, final_state, visited_states, m, n, parent):

  # Queue to implement BFS behaviour
  queue = []
  queue.append([state, i, j])

  # Marking initial state as visited
  ...
  while(len(queue)>0):
    ...
    if p>0:
      ...
      if(visited_states.get(state_to_hash(tmp_state), 1)):
        ...
        visited_states[state_to_hash(tmp_state)] = 0

        queue.append([tmp_state, p-1, q])
...
def use_bfs(initial_state, final_state):
  ...
  bfs(initial_state, init_pos[0], init_pos[1], state_to_hash(final_state), visited_states, 3,3, parent)
  
  # Calculating Time taken by the BFS Algorithm
  ...
  if parent.get(to_tuple(final_state), -1) != -1:
    ...
    print('Number of steps taken to reach the Final State by BFS: ' + str(step_bfs))

    # Number of states processed
    print('Number of states reached by BFS: ' + str(len(parent)))

    # Printing the time taken
    print('Total Time taken in milliseconds by BFS: ' + str(total_time_taken*1000) + '\n')
