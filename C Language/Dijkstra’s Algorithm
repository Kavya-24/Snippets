...
void dijkstra(int graph[V][V], int src) {
    ...
    for (int count = 0; count < V - 1; count++) {
        ...
        for (int v = 0; v < V; v++)
            // Update dist[v] only if it's not in sptSet, there is an edge from u to v, and total weight of the path from src to v through u is smaller than the current value of dist[v]
...
// Driver code
int main() {
    /* Let us create the example graph represented by the adjacency matrix */
    int graph[V][V] = {
        { 0, 4, 0, 0, 0, 0, 0, 8, 0 },
        { 4, 0, 8, 0, 0, 0, 0, 11, 0 },
        { 0, 8, 0, 7, 0, 4, 0, 0, 2 },
